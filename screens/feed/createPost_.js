...
export default class CreatePost_ extends Component {
  constructor(props) {
    super(props)
    this.state = {
      selectedPaths: [],
  ...
  componentDidMount() {
    ...
    // var isFromUserProfile = this.props.navigation.getParam('isFromUserProfile', false)
    // this.setState({isFromUserProfile: isFromUserProfile}, () => {
        ...
        {Platform.OS == 'ios' ? (
          <KeyboardAccessoryView alwaysVisible={true} inSafeAreaView={true}>
            <View style={{ width: '100%', left: 0, height: 50, backgroundColor: '#FFFFFF', justifyContent: 'space-between', }}>
              <View style={{ flexDirection: 'row', paddingHorizontal: 15, paddingTop: 8, borderTopWidth: 1, borderTopColor: '#DBDBDB', justifyContent:'space-between'  }}>
                <TouchableOpacity onPress={this.pickImage} style={{ flex: 0, flexDirection: 'row', padding: 5, alignItems: 'center', alignSelf: 'flex-start', backgroundColor: 'transparent' }}>
                  <Image source={Images.camera} />
                </TouchableOpacity>
                <Ripple onPress={() => (this.state.imagesPath != '' ? this.uploadImage() : this.CreatePostAPI())} style={{ width: 70, height: 35, flex: 0, flexDirection: 'row', alignSelf: 'flex-end', justifyContent:'flex-end' , backgroundColor: 'transparent' }}>
                  <Image source={this.state.title != ''?Images.gredient_bg:Images.post_gredient_bg_gray} style={{ width: 70, height: 35, borderRadius: 5 }} resizeMode={'stretch'} />
          ...
          <TouchableOpacity onPress={this.pickImage} style={{ flex: 1, flexDirection: 'row', padding: 5, alignItems: 'center', backgroundColor: 'transparent' }}>
              <Image source={Images.camera} />
            </TouchableOpacity>
            <Ripple onPress={() => (this.validate()?this.state.imagesPath != '' ? this.CreatePostAPI() : this.CreatePostAPI():null)} style={{ width: 70, height: 35, flexDirection: 'row' , backgroundColor: 'transparent' }}>
              <Image source={this.state.title != ''?Images.gredient_bg:Images.post_gredient_bg_gray} style={{ width: 70, height: 35, borderRadius: 5 }} resizeMode={'stretch'} />
              <View style={{ width: 70, height: 35, borderRadius: 5, position: 'absolute', top: 0, bottom: 0, alignSelf: 'center', justifyContent: 'center', alignItems: 'center', backgroundColor: 'transparent' }}>
                <Text style={{ alignSelf: 'center', justifyContent: 'center', alignItems: 'center', textAlign: 'center', fontSize: 14, color: 'white', fontFamily: 'SourceSansPro-Regular' }}>  POST </Text>
              </View>
    ...
    if (this.validate()) {
      ...
      WSManager.postData(URLConstants.CREATE_POST, params)
        .then(response => {
          ...
          // if (!this.state.isFromUserProfile)
          // {
