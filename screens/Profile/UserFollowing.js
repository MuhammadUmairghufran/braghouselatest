...
export default class UserFollowing extends Component {
  ...
  _onRefresh() {
    ...
    this.getContestMasterData(false, this.props.screenProps.teamDetails.team_league_id);
  }
  ...
  getContestMasterData(showLoader, teamLeagueId) {
    ...
    WSManager.postData(URLConstants.GET_FOLLOWING_LIST, params)
      .then(response => {
    ...
    if (Utility.isLoggedIn()) {
      ...
      WSManager.postData(URLConstants.TOGGLE_FOLLOW_ENTITY, params)
        .then(response => {
          ...
          this.getContestMasterData(true, this.props.screenProps.teamDetails.team_league_id);
        })
        .catch(error => {
          this.setState({ loading: false })
