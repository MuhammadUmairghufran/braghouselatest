...
export default class CreateBragScreen extends Component {
  ...
  componentDidMount() {
    ...
    var isFromUpComing = this.props.navigation.getParam('isFromUpComing', false)
    if (isFromUpComing) {
      //alert(this.props.navigation.getParam('isFromUpcoming', false))
      var conferenceList = this.props.navigation.getParam('conferenceList', [])
      var questionList = this.props.navigation.getParam('questionList', [])
      var teamList = this.props.navigation.getParam('teamList', [])
      var matchList = this.props.navigation.getParam('matchList', [])
      var selectedConference = this.props.navigation.getParam('selectedConference', '')
      var selectedTeam = this.props.navigation.getParam('selectedTeam', '')
      var selectedMatch = this.props.navigation.getParam('selectedMatch', '')


      this.setState({ conferenceList: conferenceList }, () => { })
      this.setState({ questionList: questionList }, () => { })
      this.setState({ teamList: teamList }, () => { })
      this.setState({ matchList: matchList }, () => { })
      this.setState({ selectedConference: selectedConference }, () => { })
      this.setState({ selectedTeam: selectedTeam }, () => { })
      this.setState({ selectedMatch: selectedMatch }, () => { })

      this.callWhenArriveFromUpcomingMatches(selectedConference,selectedTeam)
          ...
          <View style={CreateBragStyle.mainView}>
            <View>
              ...
              <View style={CreateBragStyle.DropDownStyle}>
                <Picker
                  iosIcon={<View style={{ position: 'absolute', left: 30, width: WW - 60, height: 40, justifyContent: 'center', alignItems: 'flex-end', marginRight: 0, }}><Image source={IMAGES.ic_down_aero}></Image></View>}
                ...
                <Picker
...

                  {
                    this.state.teamList.map((item, index) => {
                      return (<Item label={item.team_name} value={item.team_league_id} key={index} />)
                    })}
                </Picker>
              </View>
              <View style={CreateBragStyle.bottom_border} />
            ...
            <View style={CreateBragStyle.brag_button_container}>
              <TouchableOpacity onPress={() => this.createBragFromApp()}>
                <LinearGradient colors={['#1B75BC', '#9AD8DD']} style={CreateBragStyle.brag_button} >
  ...
  getAllTeams(conferenceID) {
    ...
    WSManager.postData(URLConstants.ALL_TEAMS_URL, params)
      ...
      .catch(error => {
        this.setState({ isLoading: false })
        Toaster.showLongToast('Get all teams error:' + error.message);
        this.setState({ isLoading: false })
        return error;
      });
  }
    ...
    if (this.validate()) {
      ...
      WSManager.postData(URLConstants.CREATE_BRAG_API, params)
        ...
        .catch(error => {
          this.setState({ isLoading: false })
          this.setState({ isLoading: false })
          Toaster.showLongToast('Create Brag Error:' + error.message);
...
const CustomHeader = ({ navigation }) => (
  <LinearGradient
    ...
    <View style={{ flexDirection: 'row', padding: 15, paddingTop: Platform.OS === "ios" ? 10 : 20 }}>
      <TouchableOpacity onPress={() => navigation.goBack()} style={{ flex: 1, justifyContent: 'center' }} transparent>
